...
READINESS=$(curl -s -o /dev/null -w '%{http_code}' "${AGENT_URL}/actuator/health/readiness")
assert_http_status "Test 1a: Readiness HTTP 200" "200" "${READINESS}" || true

READINESS_BODY=$(curl -fsS "${AGENT_URL}/actuator/health/readiness")
...
CHAT_RESPONSE=$(curl -fsS -X POST "${AGENT_URL}/api/v1/agent/chat" \
...
# Create a session
CREATE_RESP=$(curl -fsS -o /dev/null -w '%{http_code}' -X POST "${AGENT_URL}/api/v1/agent/session" \
    -H 'Content-Type: application/json' \
...
CREATE_SESSION_BODY=$(curl -fsS -X POST "${AGENT_URL}/api/v1/agent/session" \
    -H 'Content-Type: application/json' \
...
SESSIONS_BY_USER=$(curl -fsS "${AGENT_URL}/api/v1/agent/sessions/e2e-listing-user")
...
TOOLS_HTTP=$(curl -s -o /dev/null -w '%{http_code}' "${AGENT_URL}/api/v1/agent/tools")
assert_http_status "Test 7a: Tools endpoint HTTP 200" "200" "${TOOLS_HTTP}" || true

TOOLS_BODY=$(curl -fsS "${AGENT_URL}/api/v1/agent/tools")
...
# Store a memory fact
STORE_HTTP=$(curl -s -o /dev/null -w '%{http_code}' -X POST "${AGENT_URL}/api/v1/agent/memory" \
    -H 'Content-Type: application/json' \
...
# Recall memory
MEMORY_BODY=$(curl -fsS "${AGENT_URL}/api/v1/agent/memory")
...
SKILLS_HTTP=$(curl -s -o /dev/null -w '%{http_code}' "${AGENT_URL}/api/v1/agent/skills")
assert_http_status "Test 9a: Skills endpoint HTTP 200" "200" "${SKILLS_HTTP}" || true

SKILLS_BODY=$(curl -fsS "${AGENT_URL}/api/v1/agent/skills")
...
DOCTOR_HTTP=$(curl -s -o /dev/null -w '%{http_code}' "${AGENT_URL}/api/v1/agent/doctor")
assert_http_status "Test 10a: Doctor HTTP 200" "200" "${DOCTOR_HTTP}" || true

DOCTOR_BODY=$(curl -fsS "${AGENT_URL}/api/v1/agent/doctor")
...
CONFIG_HTTP=$(curl -s -o /dev/null -w '%{http_code}' "${AGENT_URL}/api/v1/agent/config")
assert_http_status "Test 11a: Config HTTP 200" "200" "${CONFIG_HTTP}" || true

CONFIG_BODY=$(curl -fsS "${AGENT_URL}/api/v1/agent/config")
...
MCP_HTTP=$(curl -s -o /dev/null -w '%{http_code}' "${AGENT_URL}/api/v1/mcp/servers")
assert_http_status "Test 12a: MCP servers HTTP 200" "200" "${MCP_HTTP}" || true

MCP_BODY=$(curl -fsS "${AGENT_URL}/api/v1/mcp/servers")
