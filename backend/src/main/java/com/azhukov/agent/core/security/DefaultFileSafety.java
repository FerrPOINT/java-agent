...
public class DefaultFileSafety implements FileSafety {
    ...
    //
    // Profile-scoped directories under the Hermes root that should be guarded.
    // Adding a new area here extends the guard with no other code change.
    // (Ported from Hermes file_safety.py PROFILE_SCOPED_AREAS)
    ...
    // ─── P1-10: Cross-profile write guard ───
    //
    // Ported from Hermes file_safety.py classify_cross_profile_target() and
    ...
    /**
     * Resolve the active profile name from the Hermes home directory.
     * <ul>
     ...
     *
     * <p>Returns {@code Optional.empty()} when the target is outside Hermes scope,
    ...
    public Optional<CrossProfileInfo> classifyCrossProfileTarget(Path path) {
        ...
        // Check if target is under the Hermes root at all
        Path rel;
        try {
     ...
     * or outside Hermes entirely. The caller is expected to surface the warning
     ...
     *
     * @param path the write target path
     * @return warning string if cross-profile, empty if in-scope or outside Hermes
     */
    public Optional<String> getCrossProfileWarning(Path path) {
        return classifyCrossProfileTarget(path).map(info ->
            ...
            " belongs to Hermes profile '" + info.targetProfile() + "', but the " +
    ...
    /**
     * Resolve the Hermes root directory (always the parent of any profile, never per-profile).
