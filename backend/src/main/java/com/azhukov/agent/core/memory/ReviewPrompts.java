 ...
 * Ported from the original project's background_review.py with Java-specific adaptations.
 */
...
Preference order for skills — pick the earliest that fits:
  1. UPDATE A CURRENTLY-LOADED SKILL. Check what skills were loaded via /skill-name or skill_view in the conversation. If one of them covers the learning, PATCH it first. It was in play; it's the right place — provided it is curator-managed. Protected and user-owned skills are off-limits however relevant; fall through when one of those is the best fit.
...
Protected skills (DO NOT edit these):
  ...
  • USER-OWNED skills — anything not curator-managed (hand-written, URL-installed, or created by a foreground agent at the user's request). Your writes to these WILL be refused, including to skills loaded or consulted this session. If one is wrong, say so in your reply and recommend 'hermes curator adopt <name>' instead.
If the only skills that need updating are protected, say
'Nothing to save.' and stop.
