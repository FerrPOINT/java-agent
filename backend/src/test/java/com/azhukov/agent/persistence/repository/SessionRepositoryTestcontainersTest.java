...
/**
 * Integration test for {@link SessionRepository} against a real PostgreSQL 16
 * instance via Testcontainers. This validates that:
 * <ul>
 *   <li>Flyway migrations apply cleanly against real PostgreSQL</li>
 *   <li>JPA queries are PostgreSQL-compatible (not just H2-compatible)</li>
 *   <li>CRUD operations work end-to-end with the real database</li>
 * </ul>
 * Tagged {@code @Tag("slow")} so it only runs via the {@code slowTest} Gradle task,
 * not in the default {@code test} task.
 */
@Tag("slow")
@SpringBootTest
...
@Transactional
class SessionRepositoryTestcontainersTest extends PostgresTestContainer {
    ...
    void deleteRemovesEntity() {
        SessionEntity session = newSession(USER_ID_1, "To Delete");
        SessionEntity saved = sessionRepository.save(session);
        UUID id = saved.getId();
    ...
    void countReturnsCorrectNumber() {
        ...
        sessionRepository.save(newSession(USER_ID_1, "Count A"));
        sessionRepository.save(newSession(USER_ID_1, "Count B"));

        assertThat(sessionRepository.count()).isEqualTo(initialCount + 2);
    }
