# the original CLI / Boot / MCP → Java Porting Summary

**Scope:** Map the Python original CLI bootstrap, entry points, CLI subcommands,
...
|------|---------|-----------|
| Console entry points (`agent`, `java-agent`, `agent-acp`) | **PORT** | Required to run the Java agent from a shell / IDE. Map to `picocli` `@Command` classes + `spring-boot-maven-plugin` executable JAR / `jpackage`. |
...
| Configuration loading (`config.yaml`, `.env`, profiles) | **PORT** | SnakeYAML + `spring-boot-starter-config` (or a thin `ConfigService`) can replace `cli.config`. Keep `AGENT_HOME` and profile resolution. |
| the original home / profile bootstrap | **PORT** | Small; port `agent_constants.get_agent_home()` and profile override ContextVar-equivalent. |
| Windows UTF-8 bootstrap (`bootstrap.py`) | **PORT** | On Windows set `PYTHONUTF8`/`PYTHONIOENCODING`. Java has no equivalent code-page problem, but document that packaged launches should set `-Dfile.encoding=UTF-8`. |
| Setup script (`setup-agent.sh`) | **PORT (as Java installer / wrapper)** | Re-implement as a cross-platform bootstrap script/JAR or keep a shell wrapper that downloads the Java runtime. Not in Java code itself. |
...
| MCP server surface (`mcp_serve.py`) | **PORT** | the original exposes messaging conversations as MCP tools. Port to an MCP Java SDK or a small stdio JSON-RPC server. |
...
[project.scripts]
agent       = "cli.main:main"
java-agent = "run_agent:main"
agent-acp   = "acp_adapter.entry:main"
```
...
|--------|------------|-----------|
| `agent` | `com.azhukov.agent.cli.AgentCli` | Picocli top-level command + subcommands. For interactive mode, delegates to `ReplShell` (JLine). |
| `java-agent` | `com.azhukov.agent.runtime.AgentMain` | Plain `main()` or Spring Boot `CommandLineRunner` that runs a single turn / batch. |
| `agent-acp` | `com.azhukov.agent.acp.AcpServerMain` | Spring Boot app (or plain `main`) that starts the ACP stdio server. |
| `agent-mcp` (implied by `agent mcp serve`) | `com.azhukov.agent.mcp.McpServerMain` | Spring Boot / standalone stdio MCP server. |
...
- **`jpackage`** for native `agent` / `agent-acp` binaries on Windows/macOS/Linux.
- **Wrapper script** `setup-agent.sh` / `setup-agent.ps1` downloads a JRE and the fat JAR, symlinks launch scripts into `~/.local/bin` (mirrors current Python `setup-agent.sh`).

---

## 3. CLI parser & global flags

### 3.1 Python top-level flags (`cli/_parser.py`)
...
| `-p`, `--profile` | String | **Pre-parser flag**: consumed before Picocli runs, sets `AGENT_HOME` and strips from `sys.argv` equivalent. |
...
- `LaunchRelauncher` (port of `cli.relaunch`) records inherited flags before `exec`/`ProcessBuilder` restart.
...
|------|---------|---------------|
| `~/.agent/config.yaml` | All behavioral settings | `ConfigService` using SnakeYAML. Keep a `DEFAULT_CONFIG` map equivalent. |
| `~/.agent/.env` | API keys / secrets | `DotEnvLoader` loads into `System.getenv()` overlay or a `SecretsStore`. |
| `~/.agent/SOUL.md` | Identity / personality | Seed on first run; read into system prompt. |
| `~/.agent/active_profile` | Profile name | `ProfileResolver` reads it before `AGENT_HOME` resolution. |
| `~/.agent/.install_method` | `git`/`pip`/`docker`/`nixos`/... | `InstallMethodDetector` (port of `detect_install_method`). |
| `~/.agent/.managed` / `AGENT_MANAGED` | NixOS / Homebrew managed installs | `ManagedModeDetector`. |
| `~/.agent/.container-mode` | Container exec routing | `ContainerModeRouter` reads file and uses `ProcessBuilder` to exec into container. |

### 5.2 Config load lifecycle

Python (`cli/config.py`):

1. Resolve `AGENT_HOME` (env > platform default).
2. `ensure_agent_home()` creates subdirs (`cron`, `sessions`, `logs`, `memories`, `skills`, ...).
...
1. `AgentHomeResolver.resolve()` returns `Path`.
2. `AgentHomeInitializer.ensureHome()` creates directories with secure POSIX permissions (0700 / 0600), honoring `AGENT_UID`/`AGENT_GID` and managed-mode umask.
...
public class AgentApplication {
    public static void main(String[] args) {
        ...
        // 2. Resolve AGENT_HOME and load config.
        Path home = AgentHomeResolver.resolve();
        AgentHomeInitializer.ensureHome(home);
        ...
        app.addInitializers(ctx -> ctx.getBeanFactory().registerSingleton("agentConfig", config));
        app.run(args);
    }
}
```
...
- `cli/main.py:cmd_chat()` is the argparse dispatch entry; it imports `cli` lazily.
...
| History | JLine `LineReader` history file (`~/.agent/.cli_history`) |
...
- Exposes 10 tools matching OpenClaw's channel bridge:
  ...
  - `channels_list` (an extra).
- Reads from `~/.agent/sessions/sessions.json`, `channel_directory.json`, and the SQLite `SessionDB`.
...
| Tool handlers | `McpTool` interface; each tool maps to a method in `AgentMcpServer`. |
...
- `agent-acp` entry point loads `.env`, configures logging to stderr, parses args, and runs `acp.run_agent(AcpAgentServer())` over stdio.
- `AcpAgentServer` implements ACP lifecycle methods:
...
| `AcpAgentServer` | `AcpAgentServer` implementing the ACP schema interfaces. |
...
## 9. Bootstrap / setup (`bootstrap.py`, `setup-agent.sh`)
...
- Keep a cross-platform `setup-agent.sh` / `setup-agent.ps1` wrapper that:
  ...
  3. Creates `~/.agent` and seeds `.env`.
  4. Symlinks/copies launch scripts.
  5. Runs `java -jar java-agent.jar setup`.
...
├── AgentCli                    # Picocli top-level @Command
...
├── DotEnvLoader                 # load ~/.agent/.env
├── AgentHomeResolver           # AGENT_HOME / profile resolution
├── AgentHomeInitializer        # create dirs, secure permissions
...
├── McpServerMain                # Entry point: agent mcp serve
├── AgentMcpServer              # Tool implementations
├── EventBridge                  # SessionDB polling + waiter queue
└── McpJsonRpcTransport          # stdio JSON-RPC

com.azhukov.agent.acp
├── AcpServerMain                # Entry point: agent acp
...
1. **Precedence of `AGENT_HOME`**: env var → platform default; profile override is
...
4. **Subcommand routing**: bare `agent` defaults to `chat`; `--resume` / `--continue`
...
1. Create `AgentCli` with top-level flags and a stub `ChatCommand` that runs a
   single query or the JLine REPL.
2. Port `AgentHomeResolver`, `AgentHomeInitializer`, `ConfigService`, and
   `DotEnvLoader`.
3. Implement P0 subcommands (`chat`, `setup`, `model`, `gateway`, `mcp`, `sessions`,
   `config`, `status`, `logs`, `version`, `update`, `completion`).
4. Add the MCP server module (`AgentMcpServer`) using a small stdio JSON-RPC loop.
...
- Container-mode routing (`get_container_exec_info`) can be implemented in the
  shell wrapper or in `AgentCli` before Picocli parses args.
