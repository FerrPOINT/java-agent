...
| `hermes_cli/pty_session.py`: `PtySession`, `PtySessionRegistry`; console/PTY websocket tests | Owned, reconnectable, bounded console and PTY sessions with cursor replay and process cleanup. | `EventWebSocketHandler.java` handles generic events only; console/PTY/pub protocol does not exist. | absent | WP-9 |
