...
### Finding 1.1 — Missing `workdir` parameter (HIGH)
...
### Finding 1.2 — Missing `notify_on_complete` and `watch_patterns` for background processes (MEDIUM)
...
### Finding 1.3 — `notifyPostExecution` never called from TerminalTool (MEDIUM)
...
### Finding 1.4 — PTY output read after process exit may lose buffered data (MEDIUM)
...
### Finding 1.5 — ProcessTool output buffer not thread-safe for concurrent read/write (LOW)
...
### Finding 1.6 — ProcessTool.spawn doesn't support PTY mode (LOW)
...
### Finding 2.1 — BrowserService creates new ObjectMapper per call (MEDIUM)
...
### Finding 2.2 — BrowserTypeTool has XSS/injection vulnerability in script construction (CRITICAL)
...
### Finding 2.3 — BrowserPressTool has similar injection vulnerability (HIGH)
...
### Finding 2.4 — BrowserScrollTool has integer injection (LOW)
...
### Finding 2.5 — BrowserSnapshotTool is shallow — hardcoded JS instead of accessibility tree (HIGH)
...
### Finding 2.6 — BrowserDialogTool uses non-standard `window.__agent_dialog` (MEDIUM)
...
### Finding 2.7 — BrowserNavigateTool ignores `waitSeconds` parameter (MEDIUM)
...
### Finding 2.8 — BrowserTypeTool ignores `clear` parameter (MEDIUM)
...
### Finding 2.9 — CdpClient has no reconnection logic (MEDIUM)
...
### Finding 2.10 — CdpClient.waitForEvent memory leak risk (LOW)
...
### Finding 2.11 — No browser tool tests for BrowserTypeTool, BrowserPressTool, BrowserScrollTool, BrowserBackTool, BrowserGetImagesTool, BrowserDialogTool (MEDIUM)
...
### Finding 3.1 — Missing `clarify` and `execute_code` in BLOCKED_TOOLSET_NAMES (HIGH)
...
### Finding 3.2 — No interrupt/pause support for subagents (MEDIUM)
...
### Finding 3.3 — No per-task timeout override in batch mode (LOW)
...
### Finding 3.4 — No `notify_on_complete` for async delegation (LOW)
...
### Finding 4.1 — MemoryTool provenance metadata not passed to provider (MEDIUM)
...
### Finding 4.2 — SkillManageTool uses class instead of record for args (LOW)
...
### Finding 4.3 — SkillViewTool resolveSkillDir hardcodes "skills" subdirectory (MEDIUM)
...
### Finding 4.4 — SkillManageTool doesn't support `absorbed_into` in update action (LOW)
...
### Finding 5.1 — DefaultContextCompressor sessionRepository is non-final, set via setter (LOW)
...
### Finding 5.2 — DefaultContextEngine prepareContext calls contextEngine.prepareContext inside itself (MEDIUM)
...
### Finding 5.3 — DefaultContextCompressor.compress doesn't handle image content in messages (MEDIUM)
...
### Finding 6.1 — No concurrent turn protection for the same session (HIGH)
...
### Finding 6.2 — Memory sync in finally block can lose data on shutdown (MEDIUM)
...
### Finding 6.3 — FallbackManager is not injected — created per-turn (LOW)
...
### Finding 7.1 — DatabaseSkillManager multi-strategy lookup doesn't search filesystem (MEDIUM)
...
### Finding 7.2 — CuratorService has no test for the actual LLM-driven consolidation (MEDIUM)
...
### Finding 8.1 — Tool refresh doesn't detect schema changes (MEDIUM)
...
### Finding 8.2 — No MCP server health monitoring (LOW)
...
### Finding 8.3 — McpServerState record doesn't update atomically (LOW)
...
### Finding 9.1 — No per-session turn queue for bot messages (MEDIUM)
...
### Finding 9.2 — No test for edited message handling edge cases (LOW)
...
### Finding 10.1 — SOUL.md path is hardcoded (LOW)
...
### Finding 10.2 — Context file scan doesn't check parent directories (MEDIUM)
...
|----------|-------|-----------|
| CRITICAL | 1 | BrowserTypeTool XSS injection (2.2) |
| HIGH | 5 | Missing workdir (1.1), BrowserPress injection (2.3), Shallow snapshot (2.5), Missing blocked tools in delegate (3.1), No concurrent turn protection (6.1) |
| MEDIUM | 16 | Missing notify_on_complete (1.2), Post-execution hooks not called (1.3), PTY data loss (1.4), ObjectMapper allocation (2.1), Dialog tool broken (2.6), Navigate ignores waitSeconds (2.7), Type ignores clear (2.8), No CDP reconnection (2.9), Missing browser tool tests (2.11), No subagent interrupt (3.2), Provenance not passed (4.1), ContextEngine self-call (5.2), Image compression (5.3), Memory sync data loss (6.2), FS skill lookup unimplemented (7.1), MCP schema change detection (8.1), Bot message queue (9.1), Context file parent dirs (10.2) |
| LOW | 10 | Buffer performance (1.5), PTY in background (1.6), Scroll injection (2.4), waitForEvent leak (2.10), Per-task timeout (3.3), SkillManageArgs class (4.2), absorbed_into in update (4.4), sessionRepository non-final (5.1), FallbackManager (6.3), MCP health (8.2), McpServerState atomicity (8.3), Edited message tests (9.2), SOUL.md path (10.1) |

**Total: 32 findings** (1 CRITICAL, 5 HIGH, 16 MEDIUM, 10 LOW)