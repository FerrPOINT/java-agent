...
| Delegate tool schema honesty | `DelegateTaskTool`, `AgentToolAnnotationParityTest` | The model-facing `delegate_task` description no longer promises Hermes automatic gateway reinjection in Java before the Java gateway consumer loop exists. It advertises the real current contract instead: background dispatch returns `run_id`/`delegation_id`, completion is persisted and emitted to the event stream, and callers can inspect it with `action=status` or `action=read`. **Update 2026-09-14:** the gateway consumer loop now exists (`DelegateReinjectionGateway`): terminal completions are claimed atomically via `claimNextPendingDelivery`, reinjected into the parent session as a bounded assistant summary plus a pending steer note, and acked through `completeDeliveryClaim`; failures release the claim and converge through the existing 8-attempt delivery ledger. The tool schema keeps the honest `action=read` hint for full results. |
...
| Profile-scoped toolsets dashboard | `ProfileService`, `ToolsetsController`, `ToolsetsControllerTest`, `ProfileServiceTest` | `/p/{profile}/v1/toolsets`, `/p/{profile}/api/tools/toolsets`, toolset toggle, and provider-selection routes no longer behave as aliases over global `AgentProperties`. Named profiles read/write `profiles/<name>/config.yaml` through `ProfileService`: `platform_toolsets.<platform>` for API/dashboard toolsets, `stt.enabled` for config-only STT, and provider choices like `web.backend` / `web.search_backend`, `tts.provider`, `image_gen.provider`, `vision.provider`, and `stt.provider`. Unknown profiles, `profile=all`, and path/query/body mismatches fail closed. Named-profile env writes are real through the masked `ProfileEnvStore` (allowlisted keys, owner-only env file, `is_set` + redacted preview, no reveal API); contract tests cover the write path and the honest 501 when the store bean is absent. Post-setup actions stay an honest 501: the interactive setup runner remains unported. |
...
1. Realtime dashboard/websocket gateway.
   ...
   - Java status: REST/SSE-ready `EventService` + `/api/events` profile-scoped replay core is implemented, and `/api/events` now also streams replay/live events over WebSocket. `/api/audio/speak-stream` has a bounded real relay over the existing Java TTS service instead of a permanent fallback stub. Current dashboard WebSocket routes have a Spring handshake Host/Origin guard. `/api/console` now has a bounded REST command-task surface (start/status/output/kill behind `CommandGuard` with redaction, see `ConsoleController`) plus live output streaming over WebSocket: `/api/console/ws?id=<task>` (`ConsoleWebSocketHandler`) streams redacted delta frames and a terminal `exit` frame behind the same dashboard handshake guard. **Update 2026-09-14 (WP-9, ADR-015):** interactive PTY is REAL (`script` transport, bounded ring, cursor reconnect, idle TTL, process-group cleanup, fail-closed when `/usr/bin/script` is absent — see `PtySessionService`); `/api/console` command tasks are durable with monotonic cursor replay (`console_tasks`/`console_task_output`, V61); `/api/pub` sidecar channels are real (`PubChannelRegistry`: profile ACL, bounded retention, drop-oldest backpressure, cursor replay). Still open: confirmation state and raw PCM streaming (depends on provider matrix).
...
3. Delegate async orchestration beyond bounded ledger.
   ...
   - Java status: bounded durable dispatch/status/read/cancel parity, atomic capacity acceptance, self-contained completion events, pending-completion restore publishing with the 48h replay age cap, delivery state/events, 8-attempt delivery convergence, service-level claim/ack/release/drop, and honest model-facing schema text are implemented. The baseline gateway consumer loop is now implemented (`DelegateReinjectionGateway`): a scheduled loop claims restorable terminal completions, reinjects bounded summaries into the parent session (message history + steer buffer), and acks/releases through the delivery ledger. Hermes completion-batch coalescing is now implemented in `DelegateReinjectionGateway`: each tick claims the pending backlog, groups completions per parent session (`DelegateCompletionBatcher`, cap 5 per note) and reinjects a single synthetic note per session; single-run batches keep the detailed Hermes-parity format. The progress-based stalled-run monitor is now implemented (docs/35 WP-1 item 8): `delegated_task_runs.last_progress_at`/`last_progress_summary`/`stalled_diagnostic_at` (миграция V64), `recordProgress` coalesces consecutive identical summaries, `StalledRunMonitor` emits a one-time diagnostic for running runs without fresh progress (fallback `started_at`) and never auto-fails without a confirmed timeout policy.
...
4. Browser cloud/hybrid control.
   ...
   - Java status: browser/CDP tools exist, `browser_vision` now returns `screenshot_path`/`MEDIA:<path>` metadata for multimodal/gateway handoff while preserving the legacy `BrowserService.screenshot()` data URL for API vision, explicit non-local `browser.cloud-provider` values fail closed instead of silently falling back to local CDP, and raw CDP/eval URL-safety bypasses are guarded. **Update 2026-09-14 (WP-8):** real provider routing exists — `BrowserBackend` port + `BrowserBackendRouter` (EXPLICIT `agent.browser.cloud-provider` selection only; runtime `backend` hint never routes; unknown providers fail closed; shared SSRF/website-policy guard pre-dispatch and post-redirect) + `ExtensionBrowserBackend` (user's own browser via authorized extension bridge). Camofox/browser-use remain separate future implementations behind the port.
...
5. MCP lifecycle depth.
    ...
    - Java status: `McpLifecycleManager`, `McpServerService`, and `McpTool` provide a base; utility capability gating, untrusted-server readOnlyHint approval gating, per-server `enabled=false`, native tool include/exclude filters, hard caps for pathological text/structured results, per-server circuit breaker short-circuiting, bounded native and generated-utility call timeout resolution, `ImageContent` MEDIA caching, `AudioContent` MEDIA caching, and embedded binary resource materialization/caps are now implemented. **Update 2026-09-14 (WP-3):** lazy persisted schema cache is real (V57 `mcp_schema_cache`, single-flight + TTL + stale-keep), dashboard MCP config CRUD/toggle are real (V57 `mcp_server_configs`, revisions, redaction), OAuth Authorization Code + PKCE flow and encrypted token storage are real (V58, AES-GCM, state-hash single-use), and catalog install writes real persisted configs. Remaining gaps: sampling/elicitation depth. *(Update 2026-09-16: POSIX-style stdio parent-death watchdogs and in-flight RPC teardown — timed-out client close/evict/reconnect — shipped in 0.1.241 and are closed; container images bundle libheif so the HEIC/AVIF transcode lane works in Docker deploys too.)*
...
6. Audio/image/vision provider ecosystem.
   ...
   - Java status: limited OpenAI TTS/STT/imagegen providers plus a WebSocket speak-stream relay over the current sync TTS service. **Update 2026-09-14 (WP-10):** a published provider capability matrix now exists (`MediaProvider` + `MediaProviderRegistry`): honest per-operation rows (MIME, limits, configured state, streaming) over the existing OpenAI/Edge beans; unavailable providers are omitted, never advertised. Raw PCM streaming stays deferred until the provider scope is explicitly accepted.

7. Execute-code runtime protocol.
   - Hermes evidence: RPC/session-kernel/code-execution mode tests.
   - Java status: local Python-oriented `ExecuteCodeTool` remains the only implemented runtime, and it now advertises/returns `execution_mode=local` and `kernel_mode=per_call`. Unsupported `session_kernel` and `remote_rpc` requests fail closed with actionable structured errors instead of being silently run as local per-call scripts. **Update 2026-09-14 (WP-7):** `session_kernel` mode is REAL (V60 `code_session_kernels`, persistent Python driver with cell protocol, state across calls, session isolation, TTL, kernel_lost semantics on restart) — no local fallback. Remote file RPC remains out of scope until explicitly accepted.

8. OpenAI rich approvals/session model.
   - Java now closes bounded `function_call` replay/id/name/argument gaps and arbitrary string session ids for OpenAI-compatible chat/responses/runs. **Update 2026-09-14 (WP-6):** Runs state is durable (V59 `openai_run_states` + event log): guarded transition table, approval→run-state mapping, cancel parity, and restart-safe SSE replay over the event sequence with cursor. Remaining: richer approval scopes.

9. CLI and Telegram remaining gateway parity.
   - CLI: file/image-drop attachments, plugin/busy-policy/runtime surface remain thinner than Hermes. **Update 2026-09-15:** attachments are end-to-end (see below); the plugin surface is a deliberate ADR-014 resolution (no dynamic JVM plugins — capability-based toolset availability instead), not an open gap. **Update 2026-09-15 (WP-11 tail):** the attachment contract is real end-to-end — `/attach <path> [photo|voice|video|file]` validates path/symlink/size/extension/disposition, uploads via `POST /api/v1/attachments` (origin=cli) and the chat request carries structured `attachments` refs; `/attachments` lists staged ids, `/detach` removes them. `/image` reuses the same validated lane.
   - Telegram: native relay/media contracts, reaction/prompt ops, DM topics, active-session recovery, background slash behavior, media re-delivery/dedupe, and STT/video/audio document parity remain open. **Update 2026-09-15 (WP-11 tail):** inbound media (photo/document/voice/video/sticker/audio) now registers as backend artifacts (`AttachmentApiClient`, best-effort with legacy local-path fallback), the chat request carries artifact ids, and outbound MEDIA: delivery resolves artifact ids from cache paths, skips already-delivered artifacts (V64 receipt: first platform message id wins, retry-after-ambiguous never re-sends) and marks delivered after each send. The M3 partial-tag streaming leak is fixed (trailing partial MEDIA: tail is held back during streaming; completed-text extraction still delivers). **Update 2026-09-15 (R1/R2):** active-session recovery and DM-topics depth are now implemented. Graceful shutdown marks active non-suspended sessions `resume_pending` (`SessionRecoveryService.markInterruptedOnShutdown`); at startup the bot recovers them once, routes a one-time continuation notice into the persisted topic and clears the flag (never repeats; suspended sessions untouched). The originating forum/DM topic is persisted on the session (`bot_sessions.metadata.last_thread_id`, fail-safe corrupt read) so restart-surviving sends land in the same topic. Gap 9 Telegram remainder closed.
