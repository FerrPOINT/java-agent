...
| Поле | Значение |
|---|---|
| Статус | Active product line `0.1.66`; Gradle artifacts still use `0.0.1-SNAPSHOT` |
...
- Production/development compose files, Flyway migrations and a large regression test suite.
...
|---|---|
| `docker-compose.yml` | Production-like stack on port `8080` with PostgreSQL `5432` |
| `docker-compose.local.yml` | Local dev stack on `18090`/`18091` |

Docker images use `eclipse-temurin:25-jre-noble` plus Chromium runtime dependencies.
...
├── docs/          # architecture and parity notes
├── repomix.config.json
└── docker-compose.yml
```
